...
class LazyFolderScanner(QtCore.QObject):
    ...
    def __init__(self, batch_size: int = 50, max_files: int = 1000, 
    ...
    def scan_folder(self, folder_path: Path, 
                   progress_callback: Optional[Callable[[int, int], None]] = None) -> FolderScanResult:
        """
        ...
        if total_files > self.max_files:
            return FolderScanResult([], [], total_files, 0, False, 
...
class FolderScanWorker(QtCore.QObject):
    ...
    def __init__(self, folder_path: Path, batch_size: int, max_files: int, 
    ...
    def scan(self) -> None:
        ...
        try:
            ...
            if total_files > self.max_files:
                result = FolderScanResult([], [], total_files, 0, False, 
                                        f"Too many files ({total_files} > {self.max_files})")
                self.complete.emit(result)
                self.finished.emit()
                return
