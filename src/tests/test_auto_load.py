...
def test_auto_load_first_folder_with_valid_folder():
    ...
    with patch('mdaviz.mainwindow.MainWindow._getRecentFolders', return_value=[str(test_folder)]):
        with patch('mdaviz.mainwindow.MainWindow.__init__', return_value=None):
            main_window = MainWindow()
            main_window.settings = settings
            main_window.setStatus = Mock()
...
def test_auto_load_first_folder_with_invalid_folder():
    ...
    with patch('mdaviz.mainwindow.MainWindow._getRecentFolders', return_value=[invalid_folder]):
        with patch('mdaviz.mainwindow.MainWindow.__init__', return_value=None):
            main_window = MainWindow()
            main_window.settings = settings
            main_window.setStatus = Mock()
...
def test_auto_load_first_folder_with_no_recent_folders():
    ...
    with patch('mdaviz.mainwindow.MainWindow._getRecentFolders', return_value=[]):
        with patch('mdaviz.mainwindow.MainWindow.__init__', return_value=None):
            ...
            main_window.setStatus.assert_called_with("No recent folders available for auto-loading")


def test_auto_load_disabled():
    """Test that auto-load is skipped when disabled."""
    with patch('mdaviz.mainwindow.MainWindow._getRecentFolders', return_value=["/some/folder"]):
        with patch('mdaviz.mainwindow.MainWindow.__init__', return_value=None):
            main_window = MainWindow()
            main_window.settings = settings
            main_window.setStatus = Mock()
            ...
            main_window.setStatus.assert_called_with("Auto-loading disabled by user preference") 